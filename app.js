...
function playBattleAnimation(ctx,done){
  ...
  later(()=>{actorImage.src=character.attack;actor.classList.add("striking");},3275);
  later(()=>{el.battle.classList.add("impacting");if(blocked){targetCard.classList.add("blocked");defenderActor?.classList.add("blocked");outcome.textContent=ctx.defense?`${ctx.defense.name} blocks the attack!`:"Attack causes no damage!";later(()=>{actorImage.src=character.ready;actor.classList.add("repelled");},220);}else{targetCard.classList.add(ctx.killed?"defeated":"hit");if(defenderImage)defenderImage.src=defenderCharacter.recoil;defenderActor?.classList.add(ctx.killed?"defeated":"hit");outcome.textContent=`${target.name} loses ${ctx.damage} heart${ctx.damage===1?"":"s"}!`;later(()=>{if(attackerLost){actorImage.src=character.recoil;actor.classList.add("recoiling");}else{actorImage.src=character.ready;actor.classList.add("returning");}},300);}outcome.classList.add("shown");},3450);
